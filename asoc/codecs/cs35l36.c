				 ...
				 struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		case 24:
			...
			break;
		case 32:
			asp_rx_width = CS35L36_ASP_WIDTH_32;
			break;
		default:
			return -EINVAL;
		}
		...
		case 24:
			...
			break;
		case 32:
			asp_tx_width = CS35L36_ASP_WIDTH_32;
			break;
		default:
			return -EINVAL;
		}
...
static int cs35l36_codec_probe(struct snd_soc_codec *codec)
{
	...
	regmap_update_bits(cs35l36->regmap, CS35L36_ASP_TX_PIN_CTRL,
			   CS35L36_TX_DAT_HIZ_MASK,
			   1 << CS35L36_TX_DAT_HIZ_SHIFT);
				  ...
				  struct cs35l36_platform_data *pdata)
{
	...
	pdata->amp_pcm_inv = of_property_read_bool(np, "cirrus,amp-pcm-inv");

	pdata->imon_pol_inv = of_property_read_bool(np, "cirrus,imon-pol-inv");

	pdata->vmon_pol_inv = of_property_read_bool(np, "cirrus,vmon-pol-inv");
			     ...
			     const struct i2c_device_id *id)
{
	...
	if (cs35l36->rev_id == CS35L36_REV_B0) {
		...
		if (ret < 0) {
			dev_err(dev, "Failed to apply B0 PAC patch %d\n", ret);
			goto err;
		}
